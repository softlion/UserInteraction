...
{
    private static UIColor? PlatformDefaultColor => DefaultColor?.ToPlatform();
    ...
    private static UIViewController? CurrentViewController()
    {
        try
        {
            ...
            return GetWindow().ToUIViewController(Application.Current?.Handler?.MauiContext!);
        }
        catch (Exception e)
        {
        ...
        Task.Delay((int)((apparitionDelay ?? 0)*1000+.5), dismiss).ContinueWith(t =>
        {
            if (!t.IsCanceled)
            {
                UIApplication.SharedApplication.InvokeOnMainThread(() =>
                {
                    ...
                    if (argbColor.HasValue || PlatformDefaultColor != null)
                        indicator.Color = argbColor.HasValue ? FromArgb(argbColor.Value) : PlatformDefaultColor;
        ...
        string? title = null, string? description = null, int defaultActionIndex = -1, string? cancelButton = null, string? destroyButton = null, params string[] otherButtons)
    {
        ...
        UIApplication.SharedApplication.InvokeOnMainThread(async () =>
        {
            ...
            //If it won't appear, make sure to not block the caller
